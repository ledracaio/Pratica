...
<head>
    ...
    <?php
    function calcularParcelas($valorAVista, $parcelas) {
        $taxaInicial = 0.015;
        $taxaAumento = 0.005;

        $taxaJuros = $taxaInicial + ($taxaAumento * (($parcelas / 12) - 2));
        $montante = $valorAVista * (1 + ($taxaJuros * $parcelas));
        $valorParcela = $montante / $parcelas;

        $retorno = "<p>O valor da parcela para {$parcelas} vezes é R$ " . number_format($valorParcela, 2, ',', '.') . ".</p>";
...
<body>
    <h1>Exercício 8</h1>
    <h2>Cálculo de Parcelas com Juros Simples</h2>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        calcularParcelas($valorAVista, $parcelas);
    }
    ?>
</body>
</html>
